...
class LaneATT(nn.Module):
    ...
    def nms_v1(self, output: torch.Tensor, nms_threshold: float = 50.0) -> torch.Tensor:
        """
            Apply non-maximum suppression to the proposals
        ...
        for i, line_a in enumerate(good_proposals):
            ...
            for j, line_b in enumerate(good_proposals):
                ...
                segment_a = line_a[5 + start:end]
                segment_b = line_b[5 + start:end]
    ...
    def nms_v3(self, output: torch.Tensor, nms_threshold: float = 40.0) -> torch.Tensor:
        """
            Apply non-maximum suppression to the proposals
        ...
        # Extract proposal curves (everything after column 5)
        curves = good_proposals[:, 5:]  # shape (N, L)
        A = curves[:, None, :]  # (N, 1, L)
        B = curves[None, :, :]  # (1, N, L)

        # Compute pairwise mean absolute error
        errors = torch.abs(A - B).mean(dim=-1)  # (N, N)

        # Mask of "same line" proposals
        good_proposals_mask = errors < nms_threshold
    ...
    def plot(self, output:torch.Tensor, image:np.ndarray) -> cv2.Mat:
        """
